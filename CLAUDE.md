...
4a. **API keys, wallet addresses, and private keys MUST NEVER be committed to the repo — not even temporarily, not even in test fixtures, not even in comments.** Non-negotiable. All credentials live in `secrets/*.enc.yaml` (sops-encrypted) or are injected at runtime. If you see a plaintext key anywhere in the codebase, treat it as a critical incident.
